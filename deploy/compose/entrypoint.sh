...
if [ "$#" -eq 0 ] || [ "$1" = "start" ]; then
  ...
  if [ ! -s "$pki/ca.pem" ] || [ ! -s "$pki/ca-key.pem" ] ||
     [ ! -s "$pki/server.pem" ] || [ ! -s "$pki/server-key.pem" ]; then
    umask 077
    ...
    printf '%s\n' 'subjectAltName=DNS:ryker,IP:127.0.0.1' 'extendedKeyUsage=serverAuth' >"$pki/server.ext"
