defmodule Ryker.Work.TurnChangeset do
  @moduledoc false
  ...
  @spec freeze(Turn.t(), Ryker.Work.Submission.t(), String.t(), keyword()) ::
          Ecto.Changeset.t()
  ...
  defp normalize_ledger(%{} = ledger) when map_size(ledger) > 0 do
    case Ryker.CanonicalJSON.validate(ledger, max_bytes: @ledger_bytes) do
      :ok -> ledger
      {:error, _reason} -> nil
    end
  end
