defmodule Ryker.Slack.ChannelSetup do
  @moduledoc """
  ...
  defp welcome_action(
         %{action_id: "ryker_welcome_configure"} = interaction,
         configuration_ref,
         revision,
         options
       ) do
  ...
  defp welcome_action(
         %{action_id: "ryker_welcome_view_" <> collection} = interaction,
         configuration_ref,
         revision,
         options
       )
  ...
  defp welcome_action(interaction, configuration_ref, revision, options) do
    {participation, notice} =
      case interaction.action_id do
        "ryker_welcome_be_proactive" -> {:proactive, "Update: proactive mode is on."}
        "ryker_welcome_mentions_only" -> {:mentions, "Update: I'll reply when mentioned."}
      end
  ...
  defp interaction_action("ryker_setup_participation_mentions", _session),
    do: {:ok, :participation, :mentions}

  defp interaction_action("ryker_setup_participation_proactive", _session),
    do: {:ok, :participation, :proactive}

  defp interaction_action("ryker_setup_participation_shadow", _session),
    do: {:ok, :participation, :shadow}

  defp interaction_action("ryker_setup_alerts_reply", _session),
    do: {:ok, :alerts, :reply}

  defp interaction_action("ryker_setup_alerts_offer", _session),
    do: {:ok, :alerts, :offer}

  defp interaction_action("ryker_setup_alerts_automatic", _session),
    do: {:ok, :alerts, :automatic}

  defp interaction_action("ryker_setup_audience_none", _session),
    do: {:ok, :audience, :none}

  defp interaction_action("ryker_setup_save", _session), do: {:ok, :save, nil}
  ...
  # for; they are recognized deterministically when Ryker is addressed.
