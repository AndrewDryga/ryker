defmodule Ryker.Slack.AppHome do
  @moduledoc """
  ...
  defp operator_view(snapshot) do
    ...
    blocks =
      [header("Ryker"), context("What needs you")]
      |> append_rows(
        needs_attention,
        &attention_blocks/1,
  ...
  defp collection_controls do
    actions(
      Enum.map(
        ...
        fn {kind, text} ->
          button("ryker_home_show_collection", text, collection_value(kind, 0))
        end
      )
    )
  end
  ...
  defp restricted_view do
    %{
      "blocks" => [
        header("Ryker"),
        section("Ryker is available in configured channels."),
  ...
  defp memory_review_blocks(row) do
    ...
    review_actions = [
      button("ryker_home_keep_memory_review", keep_label, review_ref),
  ...
  defp behavior_blocks(row) do
    ...
    status_button =
      case status do
        :disabled ->
          button("ryker_home_enable_behavior", "Enable", versioned_control(:behavior, row))

        _active ->
          button("ryker_home_disable_behavior", "Disable", versioned_control(:behavior, row))
      end
  ...
  defp schedule_blocks(row) do
    ...
    status_button =
      case status do
        :paused ->
          button("ryker_home_resume_schedule", "Resume", versioned_control(:schedule, row))

        :active ->
          button("ryker_home_pause_schedule", "Pause", versioned_control(:schedule, row))
  ...
  defp attention_control("retry", row),
    do: publication_button(row, "ryker_home_retry_publication", "Retry publication")

  defp attention_control("update", row),
    do: publication_button(row, "ryker_home_update_publication", "Review latest state")

  defp attention_control("discard", row) do
    publication_button(
      row,
      "ryker_home_discard_publication",
      "Discard candidate",
      destructive_confirm(
  ...
  defp attention_control("discard_workspace", row) do
    with ref when is_binary(ref) <- Map.get(row, :ref),
      ...
      button(
        ...
        "ryker-work-control:#{ref}:#{fingerprint}",
        destructive_confirm(
          "Discard this retained workspace?",
          "Ryker will obtain a fresh exact plan. Dirty work remains protected."
        )
      )
    else
      _invalid -> nil
  ...
  defp open_button(row, text \\ "Open") do
    ...
    case {url, ref} do
      {"https://slack.com/app_redirect?" <> _rest = url, ref} when is_binary(ref) ->
        button("ryker_home_open", text, ref, nil, url)
