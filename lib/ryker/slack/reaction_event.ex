defmodule Ryker.Slack.ReactionEvent do
  @moduledoc """
  Normalizes authenticated Slack emoji feedback on Ryker replies.
  ...
  @spec from_socket(map(), map()) ::
          {:ok, Ryker.Episodes.Reactions.attributes()} | :ignore | {:error, term()}
  def from_socket(
        %{
          "payload" => %{
    ...
    with :ok <- identity(identity),
         :ok <- authored_by_ryker(event, identity),
  ...
  defp authored_by_ryker(%{"item_user" => bot_user_ref}, %{bot_user_ref: bot_user_ref}),
    do: :ok

  defp authored_by_ryker(%{"item_user" => item_user}, _identity)
       when is_binary(item_user) and item_user != "",
       do: :ignore

  defp authored_by_ryker(_event, _identity),
