defmodule Mix.Tasks.Responder.LearningEval do
  @moduledoc """
  Runs a harvested conversation through production learning in an empty DB.

      MIX_ENV=test PGDATABASE=responder_learning_eval_example mix responder.learning_eval \
        --database responder_learning_eval_example --socket /absolute/coop.sock \
  ...
  only Repo and Finch, not Responder workers or transports. It refuses nonempty
  ...
  defp start_client!(options) do
    ...
    configuration = Application.fetch_env!(:responder, Repo)
    ...
    {:ok, _} = Finch.start_link(name: Responder.LearningEvalFinch)

    {:ok, client} =
      Client.new(
        socket: options[:socket],
        finch: Responder.LearningEvalFinch,
