defmodule Responder.Acceptance.Live do
  @moduledoc """
  ...
  defp with_finch(function) do
    case Finch.start_link(name: Responder.CoopFinch) do
      {:ok, pid} ->
        try do
          function.()
        after
  ...
  def deployment_ready(%{control_plane: %{ip: ip, port: port}})
    ...
    case Finch.request(request, Responder.CoopFinch, receive_timeout: 5_000) do
      {:ok, %Finch.Response{headers: headers, status: 200}} ->
        case List.keyfind(headers, "x-responder-version", 0) do
          {"x-responder-version", ^expected_version} -> :ok
