defmodule Responder.ControlPlane.Evidence do
  @moduledoc """
  ...
  @doc """
  Names the captured boundary of a full prompt. A Responder submission is what
  Responder froze and sent; it is not automatically the provider's entire
  assembled request, and worker-added instructions are never invented here.
  """
  @spec prompt_boundary(:responder_submission | :provider_request | :not_recorded) :: map()
  def prompt_boundary(boundary)
      when boundary in [:responder_submission, :provider_request, :not_recorded] do
    label =
      case boundary do
        :responder_submission -> "Submitted by Responder"
    ...
    %{
      ...
      detail:
        case boundary do
          :responder_submission ->
            "Exactly what Responder froze and sent. A worker may add its own instructions; those are not captured here."
