defmodule Responder.Evals.CoopRunner do
  @moduledoc """
  ...
  alias Responder.Evals.{AdmissionCase, WorkCase, WorldJudgeCase}
  ...
  defp execute_case(%AdmissionCase{} = eval, settings) do
    ...
    external_ref = "responder-eval:admission:#{run_ref}:#{eval.eval_id}"
    create_key = "responder:eval:admission:#{run_ref}:create"
  ...
  defp execute_case(%WorkCase{} = eval, settings) do
    run_ref = settings.id_generator.()
    external_ref = "responder-eval:work:#{run_ref}:#{eval.eval_id}"
    create_key = "responder:eval:work:#{run_ref}:create"
  ...
  defp execute_case(%WorldJudgeCase{} = eval, settings) do
    run_ref = settings.id_generator.()
    external_ref = "responder-eval:world-judge:#{run_ref}:#{eval.eval_id}"
    create_key = "responder:eval:world-judge:#{run_ref}:create"
  ...
  defp submit_turn(namespace, run_ref, session, eval, settings) do
    key = "responder:eval:#{namespace}:#{run_ref}:turn"
  ...
  defp accept_candidate(namespace, run_ref, session, turn, candidate, settings) do
    key =
      "responder:eval:#{namespace}:#{run_ref}:validate:#{candidate["attempt"]}:#{candidate["sha256"]}:accept"
  ...
  defp reject_candidate(namespace, run_ref, session, turn, candidate, violations, settings) do
    key =
      "responder:eval:#{namespace}:#{run_ref}:validate:#{candidate["attempt"]}:#{candidate["sha256"]}:reject"
    ...
    with {:ok, revision} <- revision(current),
         {:ok, %{"session" => closed}} <-
           settings.api.close_session(
             settings.client,
             session["id"],
             "responder:eval:#{namespace}:#{run_ref}:close",
             revision
           ),
         {:ok, _closed} <-
           validate_session(
  ...
  defp plan_and_discard(namespace, run_ref, session, current, settings) do
    plan_key = "responder:eval:#{namespace}:#{run_ref}:discard-plan"
    discard_key = "responder:eval:#{namespace}:#{run_ref}:discard"
