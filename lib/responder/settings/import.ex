defmodule Responder.Settings.Import do
  @moduledoc """
  ...
  alias Responder.{Bootstrap, CanonicalJSON, Defaults, Repo, Settings}
  alias Responder.Settings.Import.{Document, Participation}
  alias Responder.Settings.ImportReceipt

  @transports ~w(slack github control_plane)
  # Old section -> the owner in Responder.Defaults that now ships its tuning.
  @tuning %{
    admission: :admission,
    coop: :coop,
  ...
  @deployment [
    {[:control_plane, :ip], "RESPONDER_CONTROL_IP"},
    {[:control_plane, :port], "RESPONDER_CONTROL_PORT"},
    {[:state_tools, :ip], "RESPONDER_STATE_TOOLS_IP"},
    {[:state_tools, :port], "RESPONDER_STATE_TOOLS_PORT"},
    {[:coop_worker_gateway, :ip], "RESPONDER_WORKER_IP"},
    {[:coop_worker_gateway, :port], "RESPONDER_WORKER_PORT"},
    {[:coop_worker_gateway, :public_url], "RESPONDER_WORKER_PUBLIC_URL"},
    {[:coop_worker_gateway, :cacertfile], "RESPONDER_WORKER_CA_FILE"},
    {[:coop_worker_gateway, :ca_keyfile], "RESPONDER_WORKER_CA_KEY_FILE"},
    {[:coop_worker_gateway, :certfile], "RESPONDER_WORKER_CERT_FILE"},
    {[:coop_worker_gateway, :keyfile], "RESPONDER_WORKER_KEY_FILE"},
    {[:github, :ip], "RESPONDER_GITHUB_IP"},
    {[:github, :port], "RESPONDER_GITHUB_PORT"},
    {[:github, :api_url], "GITHUB_API_URL"},
    {[:github, :app_id], "GITHUB_APP_ID"},
    {[:webhooks, :ip], "RESPONDER_WEBHOOK_IP"},
    {[:webhooks, :port], "RESPONDER_WEBHOOK_PORT"},
    {[:emisar, :rpc_url], "EMISAR_RPC_URL"},
    {[:publication, :state_dir], "RESPONDER_STATE_DIR"}
  ]
  @core_secrets [
  ...
  defp secret_refusals(values, bootstrap) do
    Enum.flat_map(custom_secrets(values), fn {path, name} ->
      refuse(
        ...
        path,
        :is_not_registered_in_responder_webhook_secret_names
      )
    end)
  end
  ...
  defp secret_remap(values) do
    ...
    custom =
      Enum.map(custom_secrets(values), fn {path, name} ->
        %{setting: path, from: name, to: "#{name} (registered in RESPONDER_WEBHOOK_SECRET_NAMES)"}
      end)
    ...
    with false <- field in excluded,
      ...
      [
        %{
          ...
          value: value,
          shipped: ships,
          owner: "Responder.Defaults #{inspect(owner)}"
        }
      ]
    else
      _same_or_unowned -> []
