defmodule Responder.Retention.Operator do
  ...
  def rearm(session_ref, actor_ref, action_ref),
    do: change(:rearm, session_ref, actor_ref, action_ref)

  @spec discard_unmerged(String.t(), String.t(), String.t()) ::
          {:ok, map()} | {:error, term()}
  def discard_unmerged(session_ref, actor_ref, action_ref),
    do: change(:discard_unmerged, session_ref, actor_ref, action_ref)

  defp change(action, session_ref, actor_ref, action_ref) do
    with :ok <- reference(session_ref, :session_ref),
      ...
      Repo.transaction(fn ->
        change_locked(action, session_ref, actor_ref, action_ref, request_fingerprint)
      end)
      |> transaction_result()
    end
  end

  defp change_locked(action, session_ref, actor_ref, action_ref, fingerprint) do
    case Repo.one(
  ...
  defp transition(:rearm, %Session{cleanup_status: :blocked} = session)
  ...
  defp transition(:rearm, %Session{cleanup_status: :blocked}),
    do: Repo.rollback(:retention_blocked_phase_unknown)

  defp transition(:rearm, %Session{}),
    do: Repo.rollback(:retention_cleanup_not_blocked)

  defp transition(:discard_unmerged, %Session{retained_reason: "dirty"}),
  ...
  defp transition(:discard_unmerged, %Session{}),
