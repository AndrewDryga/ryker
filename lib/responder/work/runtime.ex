defmodule Responder.Work.Runtime do
  @moduledoc """
  Supervises a bounded pool of workers sharing one configured Coop endpoint.
  ...
  worker. Routing a bound Coop session across different machines is deliberately
  outside this stage; all slots in this pool must reach the same Coop daemon.
  """

  use Supervisor

  alias Responder.Coop.Client
  alias Responder.Work.Worker

  @fields [:concurrency, :poll_interval_ms, :receive_timeout_ms, :socket, :worker_ref]
  @lease_seconds 300
  ...
  def options!(configuration) do
    ...
    case Client.new(
      ...
      {:error, reason} ->
        raise ArgumentError, "invalid work Coop client: #{inspect(reason)}"
    end
  end
