defmodule Responder.Admission.Attempts do
    ...
    with :ok <- CanonicalJSON.validate(submission, max_bytes: 320 * 1_024) do
      locked(entry, settings, fn
        %Attempt{submission: nil} = attempt ->
          {:ok, _} =
            Responder.Accounting.observe_admission_in_transaction(
              entry,
              %{"state" => "requested"},
  ...
  def observe_turn(entry, turn, settings) do
    ...
    locked(entry, settings, fn attempt ->
      ...
      measurement =
        recorded
        |> Map.take(Responder.Accounting.measurement_fields())
  ...
  def committed(%Entry{} = entry) do
    :ok = Responder.Accounting.attach_admission_in_transaction(entry)
