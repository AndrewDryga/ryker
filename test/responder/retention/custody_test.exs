defmodule Responder.Retention.CustodyTest do
  ...
  test "only one worker claims an exact terminal owned session" do
    ...
    assert {:ok, claim} = Responder.Retention.Custody.claim_next("cleanup:a", 60, 900)
    ...
    assert {:ok, nil} = Responder.Retention.Custody.claim_next("cleanup:b", 60, 900)
  ...
  test "close and discard phases freeze exact revisions and survive lease turnover" do
    ...
    assert {:error, :retention_remote_session_mismatch} =
             Custody.settle_discard(
               ...
               "responder:retention:discard:#{session.id}:g1",
               "remote:wrong"
             )
  ...
  test "dirty and unpublished unmerged plans are durably retained" do
    for {suffix, dirty, unmerged, reason} <- [
      ...
      assert {:ok, claim} = Responder.Retention.Custody.claim_next("cleanup:#{suffix}", 60, 0)

      assert {:ok, _closed} =
               Responder.Retention.Custody.mark_closed(session.id, claim.lease_ref, 0)

      assert {:ok, plan_claim} =
               Responder.Retention.Custody.claim_next("cleanup:plan:#{suffix}", 60, 0)

      assert {:ok, _frozen} =
               Responder.Retention.Custody.freeze_plan_revision(
                 session.id,
                 plan_claim.lease_ref,
                 8,
                 false
  ...
  test "an operator rearms the exact blocked cleanup phase with an audited idempotency key" do
    ...
    assert Repo.aggregate(Responder.Retention.OperatorAction, :count) == 1
  ...
  test "durable evidence does not keep a terminal Coop workspace alive while unpublished review work does" do
    ...
    assert {:ok, %{session: claimed}} =
             Responder.Retention.Custody.claim_next("cleanup:evidence", 60, 0)
  ...
  test "cleanup intent makes a reopened episode rotate to a fresh immutable session" do
    ...
    assert {:ok, _claim} = Responder.Retention.Custody.claim_next("cleanup:a", 60, 900)
    ...
    assert {:ok, rotated} =
             Responder.Work.Custody.pin_episode(
               session.episode_id,
               "changed-default",
  ...
  defp insert_unpublished_publication!(session) do
    ...
    {1, nil} =
      Repo.update_all(
        from(record in Responder.State.Record, where: record.id == ^record_id),
    ...
    Repo.query!(
      ...
      VALUES ($1, $2, $3, $4, $5, 'responder', 'Review', 'Review this change.',
