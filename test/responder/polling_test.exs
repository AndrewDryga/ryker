defmodule Responder.PollingTest do
  ...
  defmodule DatabaseDispatcher do
    def run_once(options) do
      ...
      previous = Responder.Repo.put_dynamic_repo(pool)

      try do
        Responder.Repo.query!("SELECT 1", [], log: false, timeout: 50)
      rescue
        error in DBConnection.ConnectionError ->
          send(parent, {:pool_exhausted, self()})
          reraise error, __STACKTRACE__
      after
        Responder.Repo.put_dynamic_repo(previous)
      end
  ...
  # restart budget and taking Responder down. A failed cycle must wait, not crash
