defmodule Responder.Emisar.OperatorTaskTest do
  use Responder.DataCase, async: false

  alias Mix.Tasks.Responder.EmisarApproval
  ...
  test "list is read-only and reports the durable blocked-monitor inventory" do
    ...
    assert_raise Mix.Error, ~r/usage: mix responder.emisar_approval/, fn ->
  ...
  test "the operator command starts only database dependencies" do
    ...
    Application.put_env(:responder, :emisar, :must_not_start)
    Application.put_env(:responder, :slack, :must_not_start)
    Mix.Task.run("responder.emisar_approval", ["list"])
