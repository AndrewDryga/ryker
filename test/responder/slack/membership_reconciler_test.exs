defmodule Responder.Slack.MembershipReconcilerTest do
  ...
  test "a missed absent-to-present transition creates and posts exactly one durable setup" do
    agent =
      start_supervised!({Agent, fn -> %{channels: ["C456"], deliveries: %{}, posts: 0} end})

    options = options(agent)

    assert MembershipReconciler.run_once(options) == {:ok, %{channels: 1, prompted: 1}}
  ...
  test "managed incident rooms are excluded from generic channel onboarding" do
    agent =
      start_supervised!(
        {Agent, fn -> %{channels: ["C456", "CINCIDENT"], deliveries: %{}, posts: 0} end}
      )
    ...
    assert MembershipReconciler.run_once(options) == {:ok, %{channels: 2, prompted: 1}}
  ...
  test "the reconciler worker polls once immediately and rejects unsafe configuration" do
    ...
    valid =
      agent
      |> options()
      |> Map.merge(%{interval_ms: 30_000, name: :membership_reconciler_test})
  ...
  test "already configured channels are counted without another setup prompt" do
    ...
    assert MembershipReconciler.run_once(configured) == {:ok, %{channels: 1, prompted: 0}}
