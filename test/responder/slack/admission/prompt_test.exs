defmodule Responder.Slack.Admission.PromptTest do
  use ExUnit.Case, async: true

  alias Responder.Episodes.Episode
  alias Responder.Slack.Admission.{Candidate, Context, Decision, Prompt}
  alias Responder.Slack.Inbox.Entry
  alias Responder.Slack.Input

  test "gives every provider the same generic decision contract" do
    input = input!()
    ...
    assert request["response_schema"] == Decision.json_schema()
    assert request["instructions"] =~ "Interpret the message itself"
    assert request["instructions"] =~ "Never ignore a request"
    assert request["instructions"] =~ "at Responder."
