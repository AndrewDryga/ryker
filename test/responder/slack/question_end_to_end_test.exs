defmodule Responder.Slack.QuestionEndToEndTest do
  ...
  alias Responder.Slack.{InteractionAudit, InteractionFeedbackWorker}
    ...
    assert {:ok, request} =
             Records.create(Records.token(work_claim.turn), "rollout-choice", "input_request", %{
               ...
               "remember" => %{
                 ...
                 "applicability" => "Responder canary deployments"
               }
             })
  ...
  defp submit_answer!(:choice, request, _occurred_at) do
    ...
    envelope = %{
      ...
      "payload" => %{
        ...
        "state" => %{
          "values" => %{
            request.ref => %{"responder_question_choice" => Map.delete(radio, "action_id")}
          }
        }
      }
    }
    ...
    submit =
      ...
      |> put_in(["payload", "actions"], [
        %{"type" => "button", "action_id" => "responder_submit_input", "value" => request.ref}
      ])
