defmodule Responder.BootstrapTest do
  use ExUnit.Case, async: true

  alias Responder.Bootstrap

  @database "ecto://responder:database-secret@localhost/responder"
  ...
  test "a database-only bootstrap does not invent integrations, identities or keys" do
    ...
    assert settings.storage_root == "/var/lib/responder"
  ...
  test "explicit bootstrap overrides are bounded and do not become product settings" do
    settings =
      Bootstrap.load!(
        environment(%{
          "POOL_SIZE" => "7",
          "RESPONDER_CONTROL_IP" => "::1",
          "RESPONDER_CONTROL_PORT" => "54321",
          "RESPONDER_STATE_DIR" => "/srv/responder",
          "GITHUB_APP_ID" => "123",
          "GITHUB_API_URL" => "https://github.example/api/v3",
          "EMISAR_RPC_URL" => "https://emisar.example/private/rpc",
          "LOG_LEVEL" => "warning",
          "RESPONDER_WORK_CONCURRENCY" => "999",
          "RESPONDER_SLACK_ENABLED" => "true"
        })
      )

    assert settings.repo[:pool_size] == 7
    assert settings.control_plane == %{ip: {0, 0, 0, 0, 0, 0, 0, 1}, port: 54_321}
    assert settings.storage_root == "/srv/responder"
  ...
  test "invalid explicit bootstrap inputs identify the variable without echoing its value" do
    invalid = [
      ...
      {"POOL_SIZE", "0"},
      {"POOL_SIZE", "20junk"},
      {"RESPONDER_CONTROL_IP", "0.0.0.0"},
      {"RESPONDER_CONTROL_PORT", "65536"},
      {"RESPONDER_STATE_TOOLS_IP", "192.0.2.1"},
      {"RESPONDER_STATE_DIR", "relative/private-secret"},
      {"GITHUB_APP_ID", "-5"},
  ...
  test "worker TLS configuration is explicit and cannot partially enable a listener" do
    transport = %{
      "RESPONDER_WORKER_PUBLIC_URL" => "https://responder.example:4322",
      "RESPONDER_WORKER_CA_FILE" => "/run/credentials/ca.pem",
      "RESPONDER_WORKER_CA_KEY_FILE" => "/run/credentials/ca-key.pem",
      "RESPONDER_WORKER_CERT_FILE" => "/run/credentials/server.pem",
      "RESPONDER_WORKER_KEY_FILE" => "/run/credentials/server-key.pem"
    }

    settings = Bootstrap.load!(environment(transport))
    assert settings.worker_gateway.public_url == "https://responder.example:4322"
    assert settings.worker_gateway.cacertfile == transport["RESPONDER_WORKER_CA_FILE"]
    assert settings.worker_gateway.port == 4322
    refute Map.has_key?(settings.worker_gateway, :checkpoint_key)

    assert_raise ArgumentError, ~r/RESPONDER_WORKER_CA_KEY_FILE/, fn ->
      transport
      |> Map.delete("RESPONDER_WORKER_CA_KEY_FILE")
      |> environment()
      |> Bootstrap.load!()
    end

    assert_raise ArgumentError, ~r/RESPONDER_WORKER_PUBLIC_URL/, fn ->
      transport
      |> Map.put("RESPONDER_WORKER_PUBLIC_URL", "https://example.com/not-an-origin")
      |> environment()
      |> Bootstrap.load!()
    end
  end
  ...
  test "checkpoint custody preserves the injected key and never generates a replacement" do
    ...
    env = environment(%{"RESPONDER_CHECKPOINT_KEY" => Base.encode64(key)})
    assert Bootstrap.checkpoint_key!(env) == key
    assert Bootstrap.checkpoint_key!(env) == key

    for value <- [nil, "", Base.encode64("short"), "private-secret"] do
      env = if value, do: environment(%{"RESPONDER_CHECKPOINT_KEY" => value}), else: environment()
      error = assert_raise ArgumentError, fn -> Bootstrap.checkpoint_key!(env) end
      assert error.message =~ "RESPONDER_CHECKPOINT_KEY"
      refute error.message =~ "private-secret"
    end
  end
  ...
  test "custom webhook credentials require exact deployment registration" do
    env =
      environment(%{
        "RESPONDER_WEBHOOK_SECRET_NAMES" => "ALERTS_SIGNING_KEY,DEPLOY_SIGNING_KEY",
    ...
    for names <- ["", "A,A", "A,,B", "DATABASE_URL", "bad-name"] do
      assert_raise ArgumentError, ~r/RESPONDER_WEBHOOK_SECRET_NAMES/, fn ->
        Bootstrap.load!(environment(%{"RESPONDER_WEBHOOK_SECRET_NAMES" => names}))
      end
    end
  end
  ...
  test "secret scanning only includes configured service credentials and registered source secrets" do
    env =
      environment(%{
        ...
        "RESPONDER_WEBHOOK_SECRET_NAMES" => "ALERTS_SIGNING_KEY",
