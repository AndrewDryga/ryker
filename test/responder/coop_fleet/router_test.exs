defmodule Responder.CoopFleet.RouterTest do
  ...
  alias Responder.CoopFleet.{ArtifactTransport, ControlPlane, Enrollment, Placement, Router}
  alias Responder.Episodes
  alias Responder.Fixtures.Episodes, as: EpisodeFixtures
  alias Responder.Fixtures.WorkspaceCheckpoint, as: WorkspaceCheckpointFixture
  alias Responder.Repo
  alias Responder.Work.{Custody, Session, SessionChangeset, StateBinding}
  ...
  test "an exact SubmitTurn fence can fetch its frozen artifact but other fences cannot" do
    ...
    assert {:ok, crossed} =
             ControlPlane.enqueue_command(
               ...
               "responder:test:fence-crossed:#{Ecto.UUID.generate()}"
             )
  ...
  test "one checkpoint command stores only its verified encrypted bundle and exact replays reconcile" do
    ...
    assert {:ok, restore_command} =
             ControlPlane.enqueue_command(
               ...
               "responder:test:ensure-workspace:restore"
             )
  ...
  defp command!(kind, payload) do
    ...
    assert {:ok, command} =
             ControlPlane.enqueue_command(
               ...
               kind,
               payload,
               "responder:test:#{kind}:#{Ecto.UUID.generate()}"
             )
  ...
  defp session! do
    ...
    assert {:ok, session} =
             Custody.pin_episode(episode_id, "work-read-only", @policy_digest, "responder")
