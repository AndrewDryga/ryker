defmodule Responder.State.LearningCapacityTest do
  ...
  test "a topic above the old ceiling updates in place with complete immutable source history" do
    ...
    assert {:ok, %{status: :applied} = applied} =
             Responder.Fixtures.Learning.accept(run.id, candidate, %{})
  ...
  test "ten thousand revisions keep linear memberships and the next root is explicitly omitted" do
    ...
    original = Repo.get!(Responder.State.ConversationObservation, head.source_input_id)
    ...
    for {schema, index} <- [{Responder.State.ConversationObservation, 0}, {KnowledgeSource, 1}] do
      pairs
