defmodule Responder.State.DerivedContextTest do
  ...
  alias Responder.Work.{Custody, Final, Result, Session, Submission, SubmissionBuilder, Turn}
  ...
  test "a retained real citation cannot launder a withdrawn source into replacement context" do
    ...
    assert Repo.get!(Responder.State.Record, record.id).payload["observation"] ==
  ...
  test "records sharing one producer do not repeat its source-lineage database work" do
    ...
    for index <- 1..31 do
      record
      |> Map.from_struct()
      |> Map.take(Responder.State.Record.__schema__(:fields))
      |> Map.merge(%{
      ...
      |> then(&struct!(Responder.State.Record, &1))
      |> Repo.insert!()
    end
  ...
  defp settled_turn!(producer) do
    ...
    |> Ecto.Changeset.change(
      ...
      candidate_sha256: Responder.CanonicalJSON.digest(captured()["candidate"]),
      candidate_attempt: 1,
      ...
      delivery_fingerprint: Responder.CanonicalJSON.digest(captured()["candidate"]),
  ...
  defp prepared!(claim, record) do
    ...
    sha = Responder.CanonicalJSON.digest(candidate)
