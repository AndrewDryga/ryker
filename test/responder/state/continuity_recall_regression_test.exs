defmodule Responder.State.ContinuityRecallRegressionTest do
  ...
  defp retention!(seconds) do
    ...
    Application.put_env(:responder, :retention, %{conversation_memory_seconds: seconds})
  end
  ...
  test "an older matching continuity state survives more than 64 newer nonmatches" do
    ...
    assert [
           ...
           ] = Continuity.search_context(target, "responder", @captured_query, "workspace", 1)
  end
  ...
  test "invalid newer repository rollup scopes cannot crowd out an older public match" do
    ...
    assert [%{"source_ref" => ref}] =
             Continuity.model_context(public_target, "responder")["rollups"]
    ...
    for scope <- ["workspace", "repository"] do
      assert [
             ...
             ] = Continuity.search_context(public_target, "responder", @captured_query, scope, 1)
    end
