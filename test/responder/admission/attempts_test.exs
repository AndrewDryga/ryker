defmodule Responder.Admission.AttemptsTest do
  use Responder.DataCase, async: true
  alias Responder.Accounting.Query

  alias Responder.Admission.{Attempt, Attempts}
  ...
  test "transport retries reuse frozen request bytes after instructions change" do
    ...
    assert Repo.query!(
             ...
             [[Responder.CanonicalJSON.encode!(first), "text"]]
  ...
  test "admission usage is counted before an episode exists and survives a sparse terminal poll" do
    ...
    row = Repo.one!(Responder.Accounting.Execution)
