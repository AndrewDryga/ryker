defmodule Responder.Settings.ImportTest do
  use Responder.DataCase, async: false

  alias Responder.Bootstrap
  alias Responder.Settings
  alias Responder.Settings.Import
  alias Responder.Slack.{ChannelConfiguration, ChannelSettingOverride}
  ...
  defp variant!(replacements) do
    ...
    path =
      Path.join(System.tmp_dir!(), "responder-import-#{System.unique_integer([:positive])}.yaml")
  ...
  test "every credential name the fixed contract renames appears as a deployment remap" do
    ...
    assert remap["state_tools.token_env"] ==
             {"RESPONDER_STATE_TOOLS_TOKEN", "RESPONDER_STATE_TOOLS_TOKEN"}

    assert remap["coop_worker_gateway.checkpoint_key_env"] ==
             {"RESPONDER_CHECKPOINT_KEY", "RESPONDER_CHECKPOINT_KEY"}

    assert remap["webhooks.routes.universal.auth.secret_env"] ==
             {"GENERIC_WEBHOOK_SECRET",
              "GENERIC_WEBHOOK_SECRET (registered in RESPONDER_WEBHOOK_SECRET_NAMES)"}
  end
              ...
              {:import_refused,
               [
                 %{
                   ...
                   reason: :is_not_registered_in_responder_webhook_secret_names
                 }
               ]}}
  end
  ...
  test "each deployment input names the variable that now carries it" do
    ...
    assert deployment["RESPONDER_CONTROL_PORT"] == 4321
    assert deployment["RESPONDER_WORKER_PUBLIC_URL"] == "https://responder-worker.example.net"
    assert deployment["RESPONDER_WORKER_CA_FILE"] == "/var/lib/responder/example/worker-ca.pem"
    assert deployment["EMISAR_RPC_URL"] == "https://emisar.dev/api/mcp/rpc"
    assert deployment["RESPONDER_WEBHOOK_PORT"] == 4320
  ...
  test "an import keeps the installation identity that existing custody is keyed under" do
    ...
    assert Responder.Instructions.get(:global).text == "Existing operator guidance"
  end
  ...
  publication:
    ...
    state_dir: /var/lib/responder/example/publications
    commit_name: Example Responder
  ...
  test "a connected GitHub App imports its exact verified identities, not its names" do
    ...
    assert saved.publication.commit_email == "responder@example.com"
  ...
  test "a scan secret the deployment did not register is refused and a registered one is remapped" do
    ...
    assert %{reason: :is_not_registered_in_responder_webhook_secret_names} =
    ...
    assert to =~ "RESPONDER_WEBHOOK_SECRET_NAMES"
  end
