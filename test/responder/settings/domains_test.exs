defmodule Responder.Settings.DomainsTest do
  use Responder.DataCase, async: false

  alias Responder.Settings
  alias Responder.Settings.{Edit, PolicyBinding}
  ...
  test "collection edits share the single revision and each records one receipt", %{
    ...
    assert {:ok, saved} =
             Settings.put_repository(
               %{ref: "responder", github_repository: "emisar/responder"},
               snapshot.installation.revision,
               @actor
             )

    assert saved.installation.revision == 2
    assert [%{ref: "responder", base_branch: "main"}] = saved.repositories

    assert {:ok, saved} =
             Settings.put_repository_context(
               %{ref: "platform", primary_repository_ref: "responder", parallel_goal_limit: 2},
               2,
               @actor
             )
    ...
    assert {:error, {:settings_conflict, winner}} =
             Settings.put_repository(%{ref: "responder", base_branch: "develop"}, 2, @actor)
  ...
  test "an identical collection save is a no-op and an unknown field is refused everywhere", %{
    ...
    {:ok, saved} = Settings.put_repository(%{ref: "responder"}, 1, @actor)

    assert {:ok, ^saved} =
             Settings.put_repository(%{ref: "responder", base_branch: "main"}, 2, @actor)

    assert {:error, {:invalid_settings, [{:path, :unknown}]}} =
             Settings.put_repository(%{ref: "responder", path: "/srv/x"}, 2, @actor)
  ...
  test "a repository referenced elsewhere cannot be deleted while the reference stands" do
    ...
    assert Enum.map(saved.repositories, & &1.ref) == ["responder"]
  end
  ...
  test "policy bindings pin one policy per purpose and scope and refuse scopes that do not fit" do
    ...
    assert {:error, {:invalid_settings, [{:scope_kind, :scope}]}} =
             Settings.put_policy_binding(
               %{binding | scope_kind: :repository, scope_ref: "responder"},
               3,
               @actor
             )
  ...
  test "an imported identity is kept exactly and a second installation is refused" do
    Repo.delete_all(Responder.Settings.Installation)
    Repo.delete_all(Edit)

    assert {:ok, imported} =
             Settings.import_installation("responder-a", @actor, %{
               retention:
                 Map.put(Settings.retention_defaults(), :audit_data_seconds, 60 * 86_400),
               learning: %{enabled: true}
             })

    assert imported.installation.host_ref == "responder-a"
    ...
    assert Settings.import_installation("responder-b", @actor, %{}) ==
