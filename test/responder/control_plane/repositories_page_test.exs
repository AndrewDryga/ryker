defmodule Responder.ControlPlane.RepositoriesPageTest do
  @moduledoc """
  ...
  test "the repositories page is one toolbar, one quiet count and one comparison table in that order" do
    ...
    # a row of bold counts and two disclosures, under a "Where Responder can
  ...
  test "a repository row compares its counts and last revision, with access and worker evidence on demand" do
    ...
    assert LazyHTML.query(identity, "a[href='/activity?repository=responder']") |> LazyHTML.text() =~
             "View requests"
    ...
    assert text =~ "contributor responder-write"
