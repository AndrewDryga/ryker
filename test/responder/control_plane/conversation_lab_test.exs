defmodule Responder.ControlPlane.ConversationLabTest do
  ...
  test "the lab projects only its durable operator inputs and accepted visible replies" do
    ...
    assert [task_card, memory_card, schedule_card, preference_card] = responder.cards
    assert task_card.title == "Finish Lab parity"
    assert task_card.action == :confirm_task
    assert {"Repository", "responder"} in task_card.details
    ...
    turn_count = Repo.aggregate(Responder.Work.Turn, :count)
    ...
    assert Repo.aggregate(Responder.Work.Turn, :count) == turn_count
    ...
    updated_responder = Enum.find(updated_conversation.messages, &(&1.actor == :responder))
    ...
    working_responder = Enum.find(working_conversation.messages, &(&1.actor == :responder))

    assert [working_task, _memory_offer, _schedule_offer, _preference_offer] =
             working_responder.cards
