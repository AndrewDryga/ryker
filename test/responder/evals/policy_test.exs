defmodule Responder.Evals.PolicyTest do
  ...
  setup do
    environment(%{
      "RESPONDER_EVAL_SOCKET" => "/var/lib/responder/eval-coop/control.sock",
      "RESPONDER_EVAL_NO_TOOLS_POLICY" => "eval-no-tools",
      "RESPONDER_EVAL_NO_TOOLS_POLICY_DIGEST" => @no_tools,
      "RESPONDER_EVAL_WORLD_POLICY" => "eval-world",
      "RESPONDER_EVAL_WORLD_POLICY_DIGEST" => @world,
      "RESPONDER_EVAL_WORLD_BASELINE_POLICY" => "eval-world-baseline",
      "RESPONDER_EVAL_WORLD_BASELINE_POLICY_DIGEST" => @baseline
    })
  end
  ...
  test "each live eval lane receives only its dedicated authority" do
    ...
    assert Policy.socket() == {:ok, "/var/lib/responder/eval-coop/control.sock"}
  end

  test "a world lane may omit a baseline until paired qualification is requested" do
    System.delete_env("RESPONDER_EVAL_WORLD_BASELINE_POLICY")
  ...
  test "an eval can never borrow a reviewed production authority" do
    ...
    {:ok, _} = Settings.put_repository(%{ref: "responder"}, 1, "control-plane:local")
    ...
    System.put_env("RESPONDER_EVAL_NO_TOOLS_POLICY", "responder-conversation-v1")
  ...
  test "missing, malformed or crossed eval authority fails closed" do
    System.delete_env("RESPONDER_EVAL_NO_TOOLS_POLICY")
    ...
    environment(%{
      "RESPONDER_EVAL_NO_TOOLS_POLICY" => "eval-no-tools",
      "RESPONDER_EVAL_NO_TOOLS_POLICY_DIGEST" => "not-a-digest"
    })

    assert Policy.for_kind(:admission) == {:error, :invalid_model_eval_policy_digest}

    environment(%{"RESPONDER_EVAL_NO_TOOLS_POLICY_DIGEST" => @no_tools})
    environment(%{"RESPONDER_EVAL_WORLD_POLICY_DIGEST" => @no_tools})
    assert Policy.for_kind(:world) == {:error, :model_eval_policies_must_be_distinct}

    System.delete_env("RESPONDER_EVAL_SOCKET")
    assert Policy.socket() == {:error, :model_eval_socket_not_configured}
    System.put_env("RESPONDER_EVAL_SOCKET", "relative/control.sock")
