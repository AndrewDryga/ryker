defmodule Ryker.Ingress.InboxTest do
  ...
  test "Slack addressing belongs to the first receipt without changing source identity" do
    # Admission could see a human mention but not which user was Ryker.
    input = input!()
    fingerprint = Ryker.Ingress.Input.fingerprint(input)
    ...
    assert Ryker.Ingress.Input.fingerprint(input) == fingerprint
    refute Map.has_key?(Ryker.Ingress.Input.document(input), "slack_addressing")
  end
  ...
  test "the raw source envelope is kept beside the normalized content, never in its place" do
    ...
    # envelope is what the adapter received; the content is what Ryker made
    ...
    assert entry.event_fingerprint == Ryker.Ingress.Input.fingerprint(input)
    ...
    assert bare.event_fingerprint ==
             Ryker.Ingress.Input.fingerprint(
