defmodule Ryker.Learning.UsageBackfillTest do
  ...
  import Ecto.Query
  alias Mix.Tasks.Ryker.BackfillLearningUsage
  alias Ryker.Accounting.Execution
  alias Ryker.Fixtures.Learning, as: Fixtures
  alias Ryker.Ingress.Inbox.Entry
  alias Ryker.Learning.{Batch, Dispatcher, UsageBackfill}
  ...
  defmodule ForgottenSessionAPI do
    def get_session(_client, _session_id),
      do: {:error, {:coop_worker_command_timeout, "ryker:fleet:read:get_session:1"}}
  end
  ...
  test "the backfill command takes no target but the apply switch" do
    ...
    for arguments <- [
          ["--config", "/etc/ryker/ryker-elixir.yaml"],
          ["all"],
          ["--apply", "--apply"]
        ] do
