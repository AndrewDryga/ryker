defmodule Ryker.Publication.GitTest do
  ...
  test "reconstructs the reviewed tree and pushes one lease-protected branch" do
    ...
    try do
      ...
      assert String.starts_with?(result.branch_ref, "refs/heads/ryker/")
  ...
  test "updates an existing draft only with the exact observed remote-head lease" do
    ...
    request =
      request!("diff --git a/a b/a\n+change\n", %{
        branch_ref: "refs/heads/ryker/existing-draft",
    ...
    try do
      assert {:ok, %{branch_ref: "refs/heads/ryker/existing-draft"}} =
               Git.publish_candidate(
                 request,
                 %{
                   base_branch: "main",
      ...
      assert "--force-with-lease=refs/heads/ryker/existing-draft:#{observed_head}" in push
    after
  ...
  test "a first-publish branch race retains the observed head and intended commit" do
    ...
    try do
      ...
      assert String.starts_with?(conflict["branch_ref"], "refs/heads/ryker/")
  ...
  test "an existing human-owned branch is rejected before Git runs" do
    ...
    try do
      assert Git.publish_candidate(
               request,
               %{base_branch: "main", github_repository: "acme/ryker", path: repository},
               %{
                 branch_prefix: "ryker",
                 command: Command,
                 commit_email: "ryker@emisar.dev",
  ...
  # push anywhere outside the Ryker-owned prefix, before Git runs.
  ...
  defp temp_directory!(suffix) do
    path = Path.join(System.tmp_dir!(), "ryker-publication-#{suffix}-#{Ecto.UUID.generate()}")
    File.mkdir!(path)
    path
  end
