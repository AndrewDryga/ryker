defmodule Ryker.State.KnowledgeConcurrencyTest do
  ...
  defp cleanup_learning(entries) do
    ...
    Repo.delete_all(from(s in Ryker.Work.Session, where: s.learning_run_id in ^run_ids))
    ...
    Repo.delete_all(from(e in Ryker.Episodes.Event, where: e.episode_id in ^ids))
      ...
      first =
        unboxed_task(fn ->
          safely_update(fn ->
            ...
            receive do: (:update ->
                           Ryker.Fixtures.Knowledge.record_topic(
                             entry_a,
                             proposal(item_a),
                             offered_a
                           ))
