defmodule Ryker.Work.ExecutorTest do
  ...
  for origin <- [:automated, :human_followup, :human_followup_rotated] do
    test "an unchanged #{origin} notification settles into an event-only wait without a Slack delivery" do
      ...
      episode = Repo.get!(Ryker.Episodes.Episode, claim.episode.id)
  ...
  test "accepted writable work is checkpointed before local delivery custody is released" do
    ...
    assert key =~ "ryker:work:checkpoint:#{claim.turn.id}:a1:"
    ...
    refute Repo.exists?(
             from(publication in Ryker.Publication.Publication,
    ...
    settled = Repo.get!(Ryker.Work.Turn, claim.turn.id)
  ...
  for {label, evidence} <- [
    ...
    test "the evidence hook changes nothing about the turn when there is #{label}" do
      ...
      # Ryker.CoopFleet.SessionEvidenceTest.
      ...
      assert turn.submission_fingerprint == Ryker.CanonicalJSON.digest(frozen.submission)
  ...
  test "confirmed completion is durable before fallible host finalization starts" do
    ...
    assert Repo.get!(Ryker.Work.Turn, claim.turn.id).cancellation_intent == nil
  end
  ...
  test "one Work session freezes and sends its dedicated state-tools binding" do
    ...
    assert binding["endpoint"] == "https://ryker.example/v1/state-tools/mcp"
    ...
    persisted = Repo.get!(Ryker.Work.Turn, claim.turn.id)
  ...
  test "the exact authenticated input artifact reaches Coop from durable custody" do
    ...
    persisted = Ryker.Repo.get!(Ryker.Work.Turn, claim.turn.id)
  ...
  test "one logical completion cannot combine different writable repositories" do
    ...
    Enum.each([{"api", "ryker"}, {"worker", "coop"}], fn {goal_id, repository} ->
      assert {:ok, _goal} =
  ...
  test "only the exact recoverable cleanup failure spends validation identity" do
    ...
    retry_key =
      "ryker:work:validate:#{retry_claim.turn.id}:a1:g1:#{candidate_sha256}:accept"
    ...
    retried = Ryker.Repo.get!(Ryker.Work.Turn, retry_claim.turn.id)
    ...
    blocked_key =
      "ryker:work:validate:#{blocked_claim.turn.id}:a1:g1:#{candidate_sha256}:accept"
    ...
    blocked = Ryker.Repo.get!(Ryker.Work.Turn, blocked_claim.turn.id)
  ...
  test "confirmed, uncertain, and still-running session creation have distinct custody" do
    ...
    assert Ryker.Repo.get!(Ryker.Work.Session, failed_claim.session.id).create_generation ==
             2
  ...
  test "a new logical turn rotates an exhausted Coop session before freezing its briefing" do
    ...
    persisted_turn = Ryker.Repo.get!(Ryker.Work.Turn, claim.turn.id)
  ...
  test "a new repository session waits for freshness v2 before remote creation" do
    ...
    session =
      claim.session
      |> Ecto.Changeset.change(repository_ref: "ryker")
      |> Repo.update!()
  ...
  test "a lost fleet placement rotates the immutable Work session before reconstruction" do
    ...
    persisted_turn = Repo.get!(Ryker.Work.Turn, claim.turn.id)
  ...
  test "a create the fleet never enqueued does not fence its session replacement" do
    ...
    assert FakeAPI.state(fake).create_keys == ["ryker:work:create:#{replacement.id}:g1"]
  end
  ...
  test "an evaluation freezes the exact trusted Coop workspace map before model submission" do
    ...
    assert accepted.submission_fingerprint ==
             Ryker.CanonicalJSON.digest(accepted.submission)
  ...
  test "a Coop session without owner-issued repository freshness never reaches the model" do
    claim = claim_with_bound_empty_session!("missing-workspace-freshness")
    {:ok, fake} = fake_for(claim, [reply("Must not run.")])
  ...
  test "freshness correlation distinguishes the selected head from the configured default head" do
    ...
    claim = claim_with_bound_source_session!("source-workspace-freshness", "ryker", source)
  ...
  test "a default selection needs no separate source receipt" do
    ...
    source = %{"kind" => "default"}
    claim = claim_with_bound_source_session!("source-default-workspace", "ryker", source)
  ...
  test "a selected commit without fresh remote evidence never reaches the model" do
    ...
    claim = claim_with_bound_source_session!("source-commit-unproven", "ryker", source)
    {:ok, fake} = fake_for(claim, [reply("Must not run.")])
  ...
  test "a selected commit proven by the configured remote reaches the model" do
    ...
    default_head = String.duplicate("2", 40)
    source = %{"kind" => "commit", "sha" => selected}
    claim = claim_with_bound_source_session!("source-commit-proven", "ryker", source)
  ...
  test "a binding that answers another request or another head never reaches the model" do
    ...
    for {suffix, returned, base_commit} <- cases do
      claim = claim_with_bound_source_session!("source-#{suffix}", "ryker", source)
      {:ok, fake} = fake_for(claim, [reply("Must not run.")])
  ...
  test "a historical session without a persisted selector still runs" do
    ...
    # Coop already bound this session to a branch before Ryker persisted
  ...
  test "a submit revision conflict spends only the submit generation" do
    ...
    reloaded = Ryker.Repo.get!(Ryker.Work.Turn, claim.turn.id)
  ...
  test "a queued remote turn has a bounded wait and renews a long-lived local lease" do
    ...
    reloaded = Ryker.Repo.get!(Ryker.Work.Turn, work.turn.id)
