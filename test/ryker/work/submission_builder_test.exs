defmodule Ryker.Work.SubmissionBuilderTest do
  ...
  test "a continuation in the same Coop session sends a delta instead of the briefing again" do
    ...
    frozen = Ryker.Repo.get!(Ryker.Work.Turn, first.turn.id)
  ...
  test "a fresh session reserves required instructions before shedding consumed history" do
    ...
    assert {:ok, _} = Ryker.Instructions.save(:global, global, 0, "operator:test")
  ...
  defp assert_bounded_with_notes(claim, input_key, text) do
    ...
    assert byte_size(Ryker.CanonicalJSON.encode!(submission["context"])) <= 160 * 1_024
    ...
    assert {:ok, memory} =
             %{
               answer_provenance: %{
                 ...
                 "question_sha256" => Ryker.CanonicalJSON.digest("Which GCP project?"),
                 "source_revision" => 1,
                 "answer_sha256" => Ryker.CanonicalJSON.digest("emisar-project-qa")
               },
               ...
               ref: "memory:#{id}",
               scope_kind: :global,
               scope_ref: "installation:#{Ryker.CanonicalJSON.digest(applicability)}",
