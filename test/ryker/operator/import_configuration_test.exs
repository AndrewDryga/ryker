defmodule Ryker.Operator.ImportConfigurationTest do
  ...
  alias Mix.Tasks.Ryker.ImportConfiguration
  ...
  setup do
    ...
    put_variable("DATABASE_URL", "ecto://ryker:import-test@127.0.0.1/ryker_import_test")
    put_variable("RYKER_WEBHOOK_SECRET_NAMES", "GENERIC_WEBHOOK_SECRET")
  ...
  test "a document that is not on disk is reported instead of assumed empty" do
    missing = Path.join(System.tmp_dir!(), "ryker-import-absent-#{System.unique_integer()}")
  ...
  defp variant!(replacements) do
    ...
    path =
      Path.join(
        System.tmp_dir!(),
        "ryker-import-task-#{System.unique_integer([:positive])}.yaml"
      )
