defmodule Ryker.Coop.ClientTest do
  ...
  test "submits and fences the derived private Ryker binding on the logical turn" do
    response = %{
      "operation" => %{"id" => "op_turn", "state" => "succeeded"},
      "turn" => %{"id" => "turn_123", "session_id" => "remote_123", "state" => "queued"}
    }
    ...
    with_unix_server(planned, fn client, request ->
      assert {:ok, ^planned} =
               Client.plan_discard(
                 client,
                 "remote_123",
                 "ryker:retention:plan:session:g1",
                 8,
                 false,
                 true
               )
      ...
      assert captured.headers["idempotency-key"] == "ryker:retention:plan:session:g1"
    ...
    with_unix_server(discarded, fn client, request ->
      assert {:ok, ^discarded} =
               Client.discard_session(
                 client,
                 "remote_123",
                 "ryker:retention:discard:session:g1",
                 "op_plan"
               )

      captured = request.()
      assert captured.path == "/v1/sessions/remote_123/discard"
      assert captured.headers["idempotency-key"] == "ryker:retention:discard:session:g1"
  ...
  test "reads the exact bounded workspace changes for one session" do
    changes = %{
      ...
      "patch" => Base.encode64("diff --git a/lib/ryker.ex b/lib/ryker.ex"),
      "patch_bytes" => 52,
  ...
  test "runs an exact idempotent workspace review" do
    ...
    with_unix_server(response, fn client, request ->
      assert {:ok, ^response} =
               Client.run_review(client, "remote_123", "ryker:review:123", 9)

      captured = request.()
      assert captured.method == "POST"
      assert captured.path == "/v1/sessions/remote_123/review"
      assert captured.headers["idempotency-key"] == "ryker:review:123"
  ...
  test "reads an existing operation by its exact key" do
    ...
    with_unix_server(operation, fn client, request ->
      assert {:ok, ^operation} = Client.operation_by_key(client, "ryker:existing")
      assert request.().path == "/v1/operations?key=ryker%3Aexisting"
    end)
  end
  ...
  defp with_unix_server(response, status, function) do
    parent = self()
    socket_path = "/tmp/ryker-coop-#{System.unique_integer([:positive])}.sock"
    File.rm(socket_path)
