defmodule Ryker.Evals.WorldEvalScriptTest do
  ...
  test "shards run at once, each on its own database, ports and public URL, and merge" do
    ...
    assert {output, 0} =
             System.cmd(
               ...
               env: fixture.env ++ [{"RYKER_WORLD_EVAL_SHARDS", "4"}],
    ...
    assert [preview] = Enum.filter(calls, &(&1 =~ "ryker.eval world-shards"))
    ...
    assert [merge] = Enum.filter(calls, &(&1 =~ "ryker.eval world-merge"))
  ...
  test "a public URL without a port gets each shard's worker port" do
    ...
    assert {_output, 0} =
             System.cmd(
               "bash",
               [@script, fixture.results | @world_options],
               env: fixture.env ++ [{"RYKER_WORLD_EVAL_SHARDS", "2"}],
               stderr_to_stdout: true
             )
  ...
  test "a failed shard fails the run without a merge and still drops every campaign database" do
    ...
    assert {output, status} =
             System.cmd(
               "bash",
               [@script, fixture.results | @world_options],
               env: fixture.env ++ [{"RYKER_WORLD_EVAL_SHARDS", "3"}],
               stderr_to_stdout: true
             )
    ...
    for shards <- ["0", "abc", "65"] do
      assert {output, 2} =
               System.cmd("bash", [@script, fixture.results | @world_options],
                 env: fixture.env ++ [{"RYKER_WORLD_EVAL_SHARDS", shards}],
                 stderr_to_stdout: true
               )
  ...
  defp shard_calls(calls) do
    calls
    |> Enum.filter(&(&1 =~ "ryker.eval world "))
  ...
  defp fixture!(options) do
    root =
      Path.join(
        System.tmp_dir!(),
        "ryker-world-eval-script-#{System.unique_integer([:positive])}"
      )
    ...
    case "$*" in
      *"ryker.eval world-shards"*)
        ...
        exit 0
        ;;
      *"ryker.eval world-merge"*) exit "$FAKE_MERGE_STATUS" ;;
      *"ryker.eval world "*)
        shard=
        while [ $# -gt 0 ]; do
