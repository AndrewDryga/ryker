defmodule Ryker.Evals.WorkCaseTest do
  use ExUnit.Case, async: true

  alias Ryker.Evals.WorkCase
  ...
  test "context interpretation cases preserve harvested originals and host deduplication" do
    [root, card, completion] =
      "test/ryker/slack/fixtures/readiness_thread.json" |> File.read!() |> Jason.decode!()
  ...
  test "the recorded Work corpus compiles into the production prompt and final contract" do
    ...
    for eval <- cases do
      ...
      assert document["schema"]["title"] == "Ryker episode result"
  ...
  defp temporary_path do
    path = Path.join(System.tmp_dir!(), "ryker-work-eval-#{Ecto.UUID.generate()}.jsonl")
