defmodule Ryker.Evals.RuntimeTest do
  ...
  @certificates Path.join(System.tmp_dir!(), "ryker-eval-runtime-certificates")
  @files %{
    "RYKER_WORKER_CA_FILE" => "ca.pem",
    "RYKER_WORKER_CA_KEY_FILE" => "ca-key.pem",
    "RYKER_WORKER_CERT_FILE" => "cert.pem",
    "RYKER_WORKER_KEY_FILE" => "key.pem"
  }
  ...
  test "an unusable evaluation environment is reported with the input that is wrong" do
    System.put_env("RYKER_CHECKPOINT_KEY", "not-base64-for-thirty-two-bytes")

    assert {:error, {:model_eval_environment_invalid, message}} = Runtime.world()
    assert message =~ "RYKER_CHECKPOINT_KEY"
