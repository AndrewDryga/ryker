defmodule Ryker.Evals.WorldCaseTest do
  ...
  test "every discovery case fabricates its project world instead of a provider or a result" do
    # Production Ryker has no GCP project-listing capability, so the
  ...
  test "scheduled wakeups reject the replaced poll fallback shape and injected envelopes" do
    for injected <- [
          ...
          %{"input_profile" => %{"source" => %{"kind" => "system", "ref" => "ryker"}}}
        ] do
      {fixture, case_id, scenario_path} =
        copy_scenario_fixture!("airflow-verification-arms-wait")
  ...
  test "the scenario's Ryker state-tool catalog is an exact production snapshot" do
    assert {:ok, scenarios} = WorldCase.all(@scenario_root)
  ...
  test "a scenario may reuse one bounded in-root captured tool catalog" do
    fixture =
      Path.join(
        System.tmp_dir!(),
        "ryker-world-catalog-#{System.unique_integer([:positive])}"
      )
  ...
  test "rejects path traversal and a catalog whose digest was edited" do
    fixture =
      Path.join(System.tmp_dir!(), "ryker-world-case-#{System.unique_integer([:positive])}")
  ...
  test "a declared repository is content-addressed and pinned to one Coop commit" do
    fixture =
      Path.join(
        System.tmp_dir!(),
        "ryker-world-repository-#{System.unique_integer([:positive])}"
      )
  ...
  test "captured output artifacts are exact bounded image bytes" do
    ...
    for mutate <- [
      ...
      fixture =
        Path.join(
          System.tmp_dir!(),
          "ryker-world-artifact-#{System.unique_integer([:positive])}"
        )
  ...
  test "rejects missing, empty, and non-path scenario roots" do
    empty =
      Path.join(System.tmp_dir!(), "ryker-empty-world-#{System.unique_integer([:positive])}")
  ...
  test "rejects malformed, scalar, and oversized scenario documents at the file boundary" do
    fixture =
      Path.join(System.tmp_dir!(), "ryker-world-json-#{System.unique_integer([:positive])}")
  ...
  test "fetch propagates an invalid scenario-root error" do
    empty =
      Path.join(System.tmp_dir!(), "ryker-empty-fetch-#{System.unique_integer([:positive])}")
