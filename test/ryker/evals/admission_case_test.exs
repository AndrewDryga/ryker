defmodule Ryker.Evals.AdmissionCaseTest do
  use ExUnit.Case, async: true

  alias Ryker.Evals.AdmissionCase

  @fixture_path "test/ryker/admission/fixtures/resolved_card_continues_active_episode.json"
  ...
  test "every pending model judgment compiles into an executable opaque eval case" do
    ...
    for eval <- cases do
      ...
      assert document["schema"]["title"] == "Ryker admission decision"
  ...
  test "a harvested human-thread fixture admits only its explicit conversational same-work reply" do
    ...
    non_human_eval =
      Enum.find(
        cases,
        &(&1.fixture_path ==
            "test/ryker/admission/fixtures/terraform_lifecycle_continues_episode.json")
      )
  ...
  test "malformed manifests and candidates fail before a model score can be reported" do
    temporary = Path.join(System.tmp_dir!(), "ryker-eval-#{Ecto.UUID.generate()}.json")
  ...
  defp temporary_path do
    path = Path.join(System.tmp_dir!(), "ryker-admission-eval-#{Ecto.UUID.generate()}.json")
