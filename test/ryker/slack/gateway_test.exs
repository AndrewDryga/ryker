defmodule Ryker.Slack.GatewayTest do
    ...
    for {channel, type, expected} <- [
      ...
      envelope =
        ...
        |> put_in(["payload", "event", "blocks"], [
          %{"audience" => "mention", "ryker_user_ref" => "UOTHER"}
        ])
      ...
      assert hd(entry.content["blocks"])["ryker_user_ref"] == "UOTHER"
    end
  end
  ...
  test "successful confirmations persist card feedback before acknowledgement, including redelivery" do
    ...
    for action <-
          ~w(ryker_confirm_behavior ryker_confirm_memory ryker_confirm_schedule ryker_confirm_automation),
  ...
  test "a slash command is handled before a private Socket Mode acknowledgement payload" do
    payload = %{"response_type" => "ephemeral", "text" => "Ryker is passive."}
  ...
  test "authorized reactions on Ryker replies become passive episode feedback" do
    settings =
  ...
  defp home_interaction_envelope do
    %{
      ...
      "payload" => %{
        "actions" => [
          %{
            "action_id" => "ryker_home_pause_schedule",
            "type" => "button",
