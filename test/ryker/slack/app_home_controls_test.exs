defmodule Ryker.Slack.AppHomeControlsTest do
  ...
  test "modal, run-now, publication, retained-work, and link controls stay typed and idempotent" do
    ...
    assert {:ok, %{outcome: :discard_requested}} =
             AppHomeControls.handle(
               interaction(
                 :discard_workspace,
                 "ryker-work-control:ryker-work:episode:session:1:#{@plan_fingerprint}"
               ),
               base
             )

    assert_received {:discarded_workspace, "ryker-work:episode:session:1", @plan_fingerprint,
    ...
    assert AppHomeControls.handle(
             interaction(
               :discard_workspace,
               "ryker-work-control:other-work:episode:session:1:#{@plan_fingerprint}"
             ),
             base
           ) == {:error, :app_home_control_mismatch}

    refute_received {:discarded_workspace, "other-work:episode:session:1", _, _, _, _}
  ...
  test "typed callback errors and malformed results fail closed" do
    ...
    assert AppHomeControls.handle(
             interaction(
               :discard_workspace,
               "ryker-work-control:ryker-work:one:session:1:#{@plan_fingerprint}"
             ),
