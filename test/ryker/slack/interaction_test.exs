defmodule Ryker.Slack.InteractionTest do
  ...
  test "normalizes one host-owned task button from its exact Slack message" do
    ...
    assert interaction.action_id == "ryker_start_engineering_task"
  ...
  test "normalizes only a host-encoded durable question choice" do
    ...
    assert {:ok, interaction} = Interaction.from_socket(envelope, "T123", @now)
    assert interaction.action_id == "ryker_answer_input"
  ...
  test "only explicit submit accepts the current question's native selection" do
    ...
    submit =
      put_in(envelope(), ["payload", "actions"], [
        %{"type" => "button", "action_id" => "ryker_submit_input", "value" => ref}
      ])
    ...
    assert missing.action_id == "ryker_submit_input"
    ...
    assert missing_other.action_id == "ryker_submit_input"
  ...
  test "a rendered additional-post confirmation is accepted from the socket" do
    ...
    post =
      envelope()
      |> put_in(["payload", "actions", Access.at(0), "action_id"], "ryker_confirm_slack_post")
    ...
    assert interaction.action_id == "ryker_confirm_slack_post"
  ...
  test "normalizes only host-owned work buttons and record overflow selections" do
    ...
    assert interaction.action_id == "ryker_stop_work"
    ...
    assert interaction.action_id == "ryker_work_record"
  ...
  test "a retired Slack diff control never becomes an interaction" do
    ...
    view =
      envelope()
      |> put_in(["payload", "actions", Access.at(0), "action_id"], "ryker_view_diff")
  ...
  test "saved-entity removal controls carry one exact versioned resource" do
    ...
    for {action_id, value} <- [
          {"ryker_delete_schedule", "schedule-control:schedule:#{id}:3"},
          {"ryker_delete_behavior", "behavior-control:behavior:#{id}:1"},
          {"ryker_forget_memory", "memory:#{id}"}
        ] do
      control =
        envelope()
    ...
    for {action_id, value} <- [
          {"ryker_delete_schedule", "schedule-control:schedule:#{id}"},
          {"ryker_delete_schedule", "behavior-control:behavior:#{id}:1"},
          {"ryker_delete_behavior", "behavior-control:behavior:#{id}:0"},
          {"ryker_forget_memory", "schedule:#{id}"}
        ] do
      assert envelope()
             |> put_in(["payload", "actions", Access.at(0), "action_id"], action_id)
             |> put_in(["payload", "actions", Access.at(0), "value"], value)
  ...
  # Configure channel also lives on the private `/ryker status` reply. Its
  ...
  test "only Configure channel is accepted from an ephemeral settings reply" do
    ...
    ephemeral =
      ...
      |> put_in(["payload", "actions", Access.at(0), "action_id"], "ryker_welcome_configure")
      |> put_in(["payload", "actions", Access.at(0), "value"], "#{configuration_ref}|3")

    assert {:ok, interaction} = Interaction.from_socket(ephemeral, "T123", @now)
    assert interaction.action_id == "ryker_welcome_configure"

    for action_id <-
          ~w(ryker_welcome_be_proactive ryker_welcome_mentions_only ryker_setup_save) do
      assert ephemeral
             |> put_in(["payload", "actions", Access.at(0), "action_id"], action_id)
             |> Interaction.from_socket("T123", @now) == :ignore
    end
    ...
    assert ephemeral
           |> put_in(["payload", "actions", Access.at(0), "action_id"], "ryker_stop_work")
           |> put_in(["payload", "actions", Access.at(0), "value"], "task-card:abc123")
           |> Interaction.from_socket("T123", @now) == :ignore
  end
