defmodule Ryker.Slack.AppHomeTest do
  ...
  alias Ryker.Slack.{AppHome, AppHomeProjection, HomeEvent}
  ...
  test "publishes a bounded what-needs-me view for an operator" do
    ...
    snapshot = %{
      ...
      needs_attention: [
        ...
        %{
          ...
          ref: "ryker-work:episode-one:session:1",
    ...
    assert text =~ "ryker_home_merge_memory_review"
    assert text =~ "ryker_home_forget_memory_review"
    assert text =~ "ryker_home_edit_memory_review"
    ...
    assert text =~
             "ryker-work-control:ryker-work:episode-one:session:1:#{@plan_fingerprint}"

    assert text =~ "ryker_home_open"
    assert text =~ "https://slack.com/app_redirect?team=T123&channel=COPS"

    assert text =~ "ryker_home_show_collection"
  ...
  test "a complete list shows the items the capped sections leave out" do
    ...
    assert text =~ "ryker_home_show_dashboard"
  ...
  test "a sparse operator view stays useful and renders terminal lifecycle choices safely" do
    ...
    assert rendered =~ "ryker_home_enable_behavior"
    assert rendered =~ "ryker_home_resume_schedule"
  ...
  test "full nonoperators receive no operational details" do
    ...
    text = Jason.encode!(blocks)
    assert text =~ "Ryker is available"
