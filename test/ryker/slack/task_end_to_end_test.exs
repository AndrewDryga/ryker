defmodule Ryker.Slack.TaskEndToEndTest do
  ...
  test "a confirmed Slack task checks readiness automatically and repairs GitHub review in the same session" do
    ...
    assert rendered_offer =~ "ryker_start_engineering_task"
    assert rendered_offer =~ offer.ref
    assert rendered_offer =~ "Fix parser retries"
    assert rendered_offer =~ "ryker"
    ...
    assert updated_json =~ "ryker_stop_work"
    ...
    assert task_card["repository"] == "ryker"
    ...
    refute reviewed_blocks =~ "ryker_task_publish"
    ...
    assert updated.pull_request_url == "https://github.com/acme/ryker/pull/91"

    assert [_first_review, corrected_review] =
             Agent.get(publication_coop, & &1.review_calls)

    assert {"ryker:publication:review:#{publication.id}:g2", 7} == corrected_review
  ...
  defp github_review_post(pull_request_number, message) do
    payload = %{
      ...
      "repository" => %{"full_name" => "acme/ryker", "id" => 99},
