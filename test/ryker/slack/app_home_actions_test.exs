defmodule Ryker.Slack.AppHomeActionsTest do
  ...
  test "retained-work discard cannot cross a Slack workspace and is idempotent" do
    ...
    visibility =
      home_interaction(
        :discard_workspace,
        "ryker-work-control:#{session.external_ref}:#{session.discard_plan_fingerprint}"
      )
    ...
    assert AppHomeActions.authorize_resource(
             %{
               visibility
               | resource_ref:
                   "ryker-work-control:ryker-work:missing:session:1:#{String.duplicate("f", 64)}"
             },
             SharedAPI,
             %{shared: MapSet.new(["C456"])}
           ) == {:error, :app_home_resource_not_visible}
    ...
    assert AppHomeActions.authorize_resource(
             %{visibility | resource_ref: "ryker-work-control:malformed"},
             SharedAPI,
             %{shared: MapSet.new(["C456"])}
           ) == {:error, :app_home_resource_not_visible}
    ...
    assert Repo.get!(Ryker.Work.Session, session.id).cleanup_status == :retained
  ...
  defp retained_session!(suffix \\ "app-home", safe? \\ true) do
    ...
    |> Ecto.Changeset.change(%{
      ...
      discard_plan: plan,
      discard_plan_fingerprint: Ryker.CanonicalJSON.digest(plan),
