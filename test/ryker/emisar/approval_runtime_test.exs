defmodule Ryker.Emisar.ApprovalRuntimeTest do
  ...
  test "builds a bounded independent approval worker pool" do
    ...
    assert Enum.map(children, fn child ->
             {Ryker.Emisar.ApprovalWorker, :start_link, [worker_options]} = child.start
             worker_options[:dispatcher_options][:worker_ref]
           end) == [
             "ryker-a:emisar:slot-1",
             "ryker-a:emisar:slot-2",
             "ryker-a:emisar:slot-3"
           ]
  end
