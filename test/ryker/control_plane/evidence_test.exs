defmodule Ryker.ControlPlane.EvidenceTest do
  ...
  test "a full prompt names the boundary it was captured at" do
    ryker = Evidence.prompt_boundary(:ryker_submission)
    assert ryker.detail =~ "worker may add its own instructions"
    refute ryker.label == Evidence.prompt_boundary(:provider_request).label
