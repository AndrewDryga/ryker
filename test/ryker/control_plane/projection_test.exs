defmodule Ryker.ControlPlane.ProjectionTest do
  ...
  test "GitHub source links contain only destination metadata while the conversation retains its message" do
             ...
             GenericInput.new(%{
               ...
               content: %{
                 ...
                 "payload" => %{
                   ...
                   "repository" => %{"full_name" => "acme/ryker"}
                 }
               },
               destination: %{
    ...
    for {source_item_ref, payload, expected} <- [
          {
            ...
            "https://github.com/acme/ryker/pull/43#discussion_r9002"
          },
          {
            ...
            "https://github.com/acme/ryker/issues/44#issuecomment-9003"
          },
          {
            ...
            "https://github.com/acme/ryker/pull/45#pullrequestreview-9004"
          }
        ] do
      Repo.update_all(
        from(saved in Entry, where: saved.id == ^entry.id),
    ...
    for {source_item_ref, repository} <- [
          ...
          {"github:unsupported:9005", "acme/ryker"}
        ] do
      Repo.update_all(
        from(saved in Entry, where: saved.id == ^entry.id),
        set: [
  ...
  test "cost and power-user totals follow each triggering input without double counting" do
    ...
    for {turn, actor, message_ref} <- [
      ...
      Repo.update_all(from(t in Ryker.Work.Turn, where: t.id == ^turn.id),
        set: [turn_ref: "ingress-turn:#{entry.id}"]
      )
    end

    Repo.update_all(from(t in Ryker.Work.Turn, where: t.id == ^second.id),
      set: [usage_cost_recorded: false, usage_cost_usd: nil]
    )

    Repo.update_all(
      from(e in Ryker.Accounting.Execution,
  ...
  test "the usage period filters requests without requiring another usage dimension" do
    ...
    Repo.update_all(from(e in Ryker.Accounting.Execution, where: e.source_id == ^old.id),
  ...
  test "the channel breakdown contains only Slack without excluding Lab from overall usage" do
    ...
    for suffix <- ["lab-one", "lab-two"] do
      ...
      Repo.update_all(
        from(e in Ryker.Accounting.Execution, where: e.source_id == ^turn.id),
  ...
  test "episode trace distinguishes a pending reply from confirmed delivery" do
    ...
    Repo.update_all(from(saved in Ryker.Work.Turn, where: saved.id == ^turn.id),
      set: [
    ...
    assert Enum.any?(
             ...
             &(&1.actor == "Ryker" && &1.status == "Response sent")
           )
  end
  ...
  test "operator workbench joins incidents schedules channels and repository freshness without payload leaks" do
    ...
    Repo.update_all(
      ...
      set: [
        ...
        submission_fingerprint: Ryker.CanonicalJSON.digest(freshness)
      ]
    )
  ...
  test "usage compares work classes and response corrections without counting transport retries" do
    ...
    Repo.update_all(
      ...
      set: [
        decision_action: :start_episode,
        decision_document: decision,
        decision_fingerprint: Ryker.CanonicalJSON.digest(decision),
    ...
    Repo.update_all(
      from(turn in Ryker.Work.Turn, where: turn.id == ^measured.id),
  ...
  defp episode_key!(episode_id) do
    Ryker.Repo.get!(Ryker.Episodes.Episode, episode_id).key
  end
end
