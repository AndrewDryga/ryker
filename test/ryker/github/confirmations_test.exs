defmodule Ryker.GitHub.ConfirmationsTest do
  ...
  test "an authenticated exact-thread command confirms each offer at most once" do
    ...
    task_input = input!("/ryker confirm #{fixture.task.ref}", 9_101, 42)
    ...
    assert %Session{policy: "ryker-contributor", repository_ref: "ryker"} =
             Repo.get_by!(Session, policy: "ryker-contributor")

    assert {:ok, %{"resource_ref" => ^task_episode_ref, "status" => "duplicate"}} =
             Confirmations.apply(
               input!("/ryker confirm #{fixture.task.ref}", 9_102, 42),
               options()
             )
    ...
    assert {:ok,
            %{
             ...
             Confirmations.apply(
               input!("/ryker confirm #{fixture.memory.ref}", 9_103, 42),
               options()
             )
  ...
  test "commands copied to another thread or pointing at unsupported records fail closed" do
    ...
    assert Confirmations.apply(
             input!("/ryker confirm #{fixture.task.ref}", 9_201, 99),
             options()
           ) == {:ok, %{"status" => "invalid"}}

    unsupported = input!("/ryker confirm record:publication_offer:abc123", 9_202, 42)
    ...
    assert Confirmations.apply(input!("/ryker confirm", 9_204, 42), options()) ==
             {:ok, %{"status" => "invalid"}}
  ...
  test "confirmation configuration accepts only exact repository policies" do
    ...
    for invalid <- [
          ...
          %{
            repositories: %{
              "ryker" => %{contributor_policy: %{name: "policy", digest: "not-a-digest"}}
            }
          },
          %{repositories: %{"ryker" => %{contributor_policy: :invalid}}},
          %{repositories: %{"ryker" => :invalid}}
        ] do
  ...
  defp options do
    Confirmations.options!(%{
      repositories: %{
        "ryker" => %{
          contributor_policy: %{digest: @digest, name: "ryker-contributor"}
        }
      }
    })
  end
